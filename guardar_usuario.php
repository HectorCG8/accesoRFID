...
// Tomar datos de POST (usar null coalescing para evitar warnings)
...
if (!empty($errors)) {
    // Mostrar errores simples y volver
...
try {
    // Verificar si UID ya está registrado
    ...
    if ($stmt->fetch()) {
        ...
        echo "<div class='card'><div class='bad'><strong>La tarjeta UID ya está registrada.</strong><br> Verifica el UID o utiliza otra tarjeta.</div>";
        echo "<a class='link' href='registro.php'>Volver al formulario</a> ";
    ...
    $insert = $pdo->prepare("INSERT INTO usuarios (nombre, apellido, correo, telefono, direccion, uid) VALUES (?, ?, ?, ?, ?, ?)");
    ...
    echo "<p>Se ha guardado correctamente al usuario <strong>" . htmlspecialchars($nombre . ' ' . $apellido) . "</strong> con UID <strong>" . htmlspecialchars($uid) . "</strong>.</p>";
    echo "<div class='actions'>";
    echo "<a class='btn btn-primary' href='registro.php'>Registrar otro</a>";
    echo "<a class='btn btn-secondary' href='ver_usuarios.php'>Ver lista de usuarios</a>";
    echo "</div></div></body></html>";

} catch (PDOException $ex) {
    // Error en BD
    echo "Error al guardar: " . htmlspecialchars($ex->getMessage());
}
?>
